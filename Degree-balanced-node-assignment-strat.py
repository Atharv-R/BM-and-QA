'''
Node assignment strategy for Zephyr graph:
Degree-Balanced Assignment
...
def analyze_architecture(G, visible_nodes, hidden_nodes):
...
if __name__ == '__main__':
    print("=" * 60)
    print(f"Alternate Node Assignment Strategy for Zephyr Graph: Degree-Balanced")
    print("=" * 60)
    ...
    visible_set = assign_by_degree_balanced(G_zephyr, num_visible)
    ...
    model_path = os.path.join(data_dir, f"Degree-Balanced_{hparam_tag}_model.pt")
    torch.save({
        'model_state_dict': model.state_dict(),
        'training_history': training_history,
        'arch_name': 'Degree-Balanced',
        'hyperparams': {
    ...
    if training_history is not None and 'pcd_loss' in training_history:
        ...
        fig.suptitle(f"Training Metrics (Hidden={num_hidden})")
        fig.tight_layout()
        fig.savefig(f"Degree-Balanced_training.png")
        fig.savefig(os.path.join(data_dir, f"Degree-Balanced_{hparam_tag}_training.png"))
        print(f"Saved training plot to Degree-Balanced_training.png")
        plt.show()
    ...
    fig.suptitle(f"Degree-Balanced — Gibbs Samples (Epochs={epochs}, Hidden={num_hidden})")
    for i, ax in enumerate(axes.flat):
        if i < len(samples_np):
            ax.imshow(samples_np[i].reshape(grid_shape), cmap='gray', vmin=0, vmax=1)
        ax.axis('off')
    plt.tight_layout()
    plt.savefig(f"Degree-Balanced_samples_gibbs.png")
    plt.savefig(os.path.join(data_dir, f"Degree-Balanced_{hparam_tag}_samples_gibbs.png"))
    print(f"Saved Gibbs samples to Degree-Balanced_samples_gibbs.png")
    plt.show()
    ...
    fig_sa.suptitle(f"Degree-Balanced — SA Samples (Epochs={epochs}, Hidden={num_hidden})")
    for i, ax in enumerate(axes_sa.flat):
        if i < len(sa_np):
            ax.imshow(sa_np[i].reshape(grid_shape), cmap='gray', vmin=0, vmax=1)
        ax.axis('off')
    plt.tight_layout()
    plt.savefig(f"Degree-Balanced_samples_sa.png")
    plt.savefig(os.path.join(data_dir, f"Degree-Balanced_{hparam_tag}_samples_sa.png"))
    print(f"Saved SA samples to Degree-Balanced_samples_sa.png")
    plt.show()

    print(f"\nDegree-Balanced — Complete.")
