...
# --- Config ---
GRID_SHAPE = (13, 13)   # <-- change this to (12,12), (13,13), (14,14), etc.
...
class CustomBoltzmannMachine(nn.Module):
    ...
    def forward(self, v_data: torch.Tensor, k_steps: int = 5) -> tuple[torch.Tensor, torch.Tensor]:
        """
...
def main():
    ...
    step_size = 0.0005  # You can change this value for experiments 
    l2_amount = 0.5  # L2 regularization amount
    num_epochs = 20
    batch_size = 30  
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True)
    ...
    print("Number of visible nodes:", num_visible)
