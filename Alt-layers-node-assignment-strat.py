...
"Alternating Layers Based on X-Coordinate"
...
if __name__ == '__main__':
    print("=" * 60)
    print(f"Alternate Node Assignment Strategy for Zephyr Graph: Alternating Layers Based on X-Coordinate")
    print("=" * 60)
    ...
    visible_set = assign_alternating_layers(G_zephyr, num_visible)
    ...
    model_path = os.path.join(data_dir, f"Alt_layers_{hparam_tag}_model.pt")
    torch.save({
        'model_state_dict': model.state_dict(),
        'training_history': training_history,
        'arch_name': 'Alt_layers',
        'hyperparams': {
    ...
    if training_history is not None and 'pcd_loss' in training_history:
        ...
        fig.tight_layout()
        fig.savefig(f"Alt_layers_training.png")
        fig.savefig(os.path.join(data_dir, f"Alt_layers_{hparam_tag}_training.png"))
        print(f"Saved training plot to Alt_layers_training.png")
        plt.show()
    ...
    fig.suptitle(f"Alt Layers — Gibbs Samples (Epochs={epochs}, Hidden={num_hidden})")
    for i, ax in enumerate(axes.flat):
        if i < len(samples_np):
            ax.imshow(samples_np[i].reshape(grid_shape), cmap='gray', vmin=0, vmax=1)
        ax.axis('off')
    plt.tight_layout()
    plt.savefig(f"Alt_layers_samples_gibbs.png")
    plt.savefig(os.path.join(data_dir, f"Alt_layers_{hparam_tag}_samples_gibbs.png"))
    print(f"Saved Gibbs samples to Alt_layers_samples_gibbs.png")
    plt.show()
    ...
    fig_sa.suptitle(f"Alt Layers — SA Samples (Epochs={epochs}, Hidden={num_hidden})")
    for i, ax in enumerate(axes_sa.flat):
        if i < len(sa_np):
            ax.imshow(sa_np[i].reshape(grid_shape), cmap='gray', vmin=0, vmax=1)
        ax.axis('off')
    plt.tight_layout()
    plt.savefig(f"Alt_layers_samples_sa.png")
    plt.savefig(os.path.join(data_dir, f"Alt_layers_{hparam_tag}_samples_sa.png"))
    print(f"Saved SA samples to Alt_layers_samples_sa.png")
    plt.show()
