...
class CustomBoltzmannMachine(pnn.PyroModule):
    ...
    def _compute_local_field_v(self, v_current, h_current, unit_idx, W_vv, W_vh):
    ...
    def _compute_local_field_h(self, v_current, h_current, unit_idx, W_hh, W_vh):
    ...
    def _sample_unit_given_field(self, field, name_prefix):
        prob = torch.sigmoid(field)
        return pyro.sample(name_prefix, dist.Bernoulli(probs=prob)).to(field.device)

    def gibbs_sample_step_no_grad(self, v_current, h_current, update_v=True, update_h=True):
        ...
        with torch.no_grad():
            ...
            if update_v:
                ...
                for i in range(self.num_visible):
                    field_i = self._compute_local_field_v(v_next, h_next, i, W_vv, W_vh)
                    sample_i = self._sample_unit_given_field(field_i, name_prefix=f"v_sample_{i}")
                    v_samples.append(sample_i.unsqueeze(1))
                v_next = torch.cat(v_samples, dim=1)

            if update_h:
                h_samples = []
                for j in range(self.num_hidden):
                    field_j = self._compute_local_field_h(v_next, h_next, j, W_hh, W_vh)
                    sample_j = self._sample_unit_given_field(field_j, name_prefix=f"h_sample_{j}")
                    h_samples.append(sample_j.unsqueeze(1))
                h_next = torch.cat(h_samples, dim=1)
    ...
    def mean_field_update(self, v, h, update_v=True, update_h=True):
        W_vv, W_hh, W_vh = self._get_masked_weights()
        v_next = v.clone()
        h_next = h.clone()
    ...
    def pyro_model(self, v_data):
        pyro.module("bm_model", self)
        batch_size = v_data.size(0)
        h = pyro.sample("h", dist.Bernoulli(logits=torch.zeros(batch_size, self.num_hidden, device=device)))
        logits_v = h @ self.W_vh_raw.T + self.b_v
        v = pyro.sample("v", dist.Bernoulli(logits=logits_v), obs=v_data)
        return v

    def guide(self, v_data):
        pyro.module("bm_model", self)
        logits_h = v_data @ self.W_vh_raw + self.b_h
        pyro.sample("h", dist.Bernoulli(logits=logits_h))
